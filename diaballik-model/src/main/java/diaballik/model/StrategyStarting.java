...
/**
 * Classe representant la strategie de jeu que peu adopter une IA, et plus particulierement la strategy starting (1)
 */
public class StrategyStarting extends Strategy {
	/**
	...
	public Commande exec(final Game game) {
		//choix aleatoire de bouger la piece ou la balle
		...
		if (nb == 0) {
			...
			final ArrayList<MovePiece> deplacementvalide = new ArrayList<>();
			IntStream.rangeClosed(0, 6).forEach(i -> {
				IntStream.rangeClosed(0, 6).forEach(j -> {
					piecesAI.forEach(p -> {
						if (new MovePiece(p.getTx(), p.getTy(), i, j, game).canDo(game)) {
							final MovePiece mp = new MovePiece(p.getTx(), p.getTy(), i, j, game);
							if (mp.getTx() >= mp.tx2) {
